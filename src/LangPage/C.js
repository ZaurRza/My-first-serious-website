import "../scss/App.scss";
import "../scss/Media.scss";
import "../scss/Carousel.scss";
import "../scss/Articles.scss";
import React, { useState, useEffect } from "react";
import axios from "axios";
import { SlEye } from "react-icons/sl";
import { Formik } from "formik";
import { Shablon } from "../Pages/Shablon.js";
import { Carous } from "../Pages/Carousel.js";
import { PatternLang } from "./PatternLang";
export function C() {
  const [filter, setfilter] = useState(false);
  const [testtext, settesttext] = useState(
    "Первый день без преступлений с применением насилия"
  );
  const [testnews, settestnews] = useState([
    {
      author: "Zaur Rzaev",
      title: "Первый день без преступлений с применением насилия",
      img: "data:image/png;base64,",
      views: "4637",
      id: "1",
      lang: "C",
    },
    {
      author: "Amir Rzaev",
      title: "Изображения могут быть защищены авторским правом. ",
      img: "https://i.stack.imgur.com/hvbyk.png",
      views: "78376",
      lang: "C",
    },
    {
      author: "Zaur Rzaev",
      title: "Первый день без преступлений с применением насилия",
      img: "https://i.stack.imgur.com/N0Ir3.png",
      views: "4637",
      lang: "C",
    },
    {
      author: "Amir Rzaev",
      title: "Изображения могут быть защищены авторским правом. ",
      img: "https://habrastorage.org/getpro/habr/post_images/d07/5e5/26f/d075e526f5dce59eb0525915256cdea5.gif",
      views: "78376",
      lang: "C",
    },
    {
      author: "Zaur Rzaev",
      title: "Первый день без преступлений с применением насилия",
      img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT-Q9GdazTp2wL5ACRSlHfg2FCNpAFV5hbO0g&usqp=CAU",
      views: "4637",
      lang: "C",
    },
  ]);
  function Add(values) {
    settestnews([values, ...testnews]);
  }

  const Send = async (Email, Username, Password) => {
    if (!Email || !Username || !Password) {
      console.log("esf");
      return;
    }
    const res = await axios({
      url: "https://engine.mineapp.ru/account.php?mod=reg",
      method: "post",
      headers: { "content-type": "application/x-www-form-urlencoded" },

      params: {
        email: Email,
        login: Username,
        pass: Password,
      },
      data: null,
    });
    console.log(res);
  };
  function Create() {
    return (
      <Formik
        initialValues={{ email: "", login: "", password: "" }}
        validate={(values) => {
          const errors = {};

          return errors;
        }}
      >
        {({ values, errors, touched, handleChange, handleBlur }) => (
          <div style={{ margin: 200 }}>
            <lu>email</lu>
            <input
              type="text"
              name="email"
              onChange={handleChange}
              value={values.email}
            />

            <lu>login</lu>
            <input
              type="text"
              name="login"
              onChange={handleChange}
              value={values.login}
            />

            <lu className="signuptext">password</lu>
            <input
              className="full"
              type="password"
              name="password"
              onChange={handleChange}
              value={values.password}
            />

            <button
              onClick={() => Send(values.email, values.login, values.password)}
            >
              ОК
            </button>
          </div>
        )}
      </Formik>
    );
  }

  return (
    <div className="back">
      <div>
        <Shablon namelg="C" />
      </div>
      <div className="Home">
        <div className="boxshad"></div>
        <Carous />
        <PatternLang data={testnews} />
      </div>
    </div>
  );
}
